...
function Twitter(options) {
	...
	try {
        // use config in initialization and if not check environment variables
		var config = options || process.env;
...
Twitter.prototype.getFollowersIds = function (params, error, success, accessToken, accessTokenSecret) {
    var path = '/followers/ids.json' + this.buildQS(params);
    var url = this.baseUrl + path;
    this.doRequest(url, error, success, accessToken, accessTokenSecret);
};
...
Twitter.prototype.doRequest = function (url, error, success, accessToken, accessTokenSecret) {
    // Fix the mismatch between OAuth's  RFC3986's and Javascript's beliefs in what is right and wrong ;)
    // From https://github.com/ttezel/twit/blob/master/lib/oarequest.js

    var at = accessToken || this.accessToken;
    var ats = accessTokenSecret || this.accessTokenSecret;
    ...
    this.oauth.get(url, at, ats, function (err, body, response) {
...
Twitter.prototype.doPost = function (url, post_body, error, success, accessToken, accessTokenSecret) {
    // Fix the mismatch between OAuth's  RFC3986's and Javascript's beliefs in what is right and wrong ;)
    // From https://github.com/ttezel/twit/blob/master/lib/oarequest.js
    url = url.replace(/\!/g, "%21")
        .replace(/\'/g, "%27")
    ...
    this.oauth.post(url, at, ats, post_body, "application/x-www-form-urlencoded", function (err, body, response) {
        if (DEBUG) console.log('URL [%s]', url);
        if (!err && response.statusCode == 200) {
            success(body);
        } else {
